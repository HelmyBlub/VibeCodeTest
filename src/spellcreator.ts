...
function defaultStageRoots(): StageDraft[] {
    return [{
        element: 'none', power: 50, pitch: 75, yaw: 0,
        count: 1, spread: 0, yawSpread: 0,
        stationary: false, trigger: 'delay', triggerMs: 2000, duration: 3000,
        children: [{
            element: 'none', power: 50, pitch: 0, yaw: 0,
            count: 1, spread: 0, yawSpread: 0,
            stationary: true, trigger: 'interval', triggerMs: 500, duration: 4000,
            children: [{
                element: 'ice', power: 60, pitch: -80, yaw: 0,
                count: 3, spread: 3, yawSpread: 30,
                stationary: false, trigger: 'impact', triggerMs: 0, duration: 2000,
                children: [],
            }],
        }],
    }];
...
function draftToStage(d: StageDraft, cooldown: number, castTime: number): SpellStage {
    return {
        element: d.element, power: d.power,
        pitch: d.pitch, yaw: d.yaw,
        count: d.count, spread: d.spread, yawSpread: d.yawSpread,
        stationary: d.stationary,
        trigger: d.trigger, triggerMs: d.triggerMs, duration: d.duration,
...
function spellLabel(s: Spell, active: boolean): string {
    ...
    if (s.stages?.length) {
        return `⛓ Chain · ${ct} · ${cd} · ${s.manaCost} mp${tag}`;
    }
    ...
    const elLabel  = elements.length === 1
        ? elements[0][0].toUpperCase() + elements[0].slice(1) : 'Mixed';
...
export class SpellCreator {
    ...
    private makeChainSpell(): Spell {
        ...
        return {
            ...
            manaCost: Math.max(1, this.chainMana(this.stageRoots)),
            projectiles: [], stages,
        };
    }
    ...
    private chainMana(nodes: StageDraft[]): number {
        return nodes.reduce((sum, n) =>
            sum + (n.element !== 'none' ? calcManaCost(n.power, this.castTime) : 5) + this.chainMana(n.children), 0);
    }
    ...
    private setMode(mode: SpellMode): void {
        ...
        if (mode === 'chain') this.renderStageTree();
        else this.loadEditorUI();
        this.updatePreview();
    }
    ...
    private renderStageTree(): void {
        ...
        for (let i = 0; i < this.stageRoots.length; i++) {
            html += this.renderNode(this.stageRoots[i], [i], 0);
        }
        html += `<button class="sc-btn sc-stage-add-root" style="margin-top:8px;width:100%">+ Add root stage</button>`;
        this.stageTreeEl.innerHTML = html;
    }

    private renderNode(s: StageDraft, path: number[], depth: number): string {
        const ps      = path.join(',');
        const icon    = STAGE_ELEM_ICON[s.element];
        const canDel  = path.length > 1 || this.stageRoots.length > 1;
        const delLabel = s.children.length > 0 ? `× Del (+${this.countDescendants(s)})` : '× Del';
  ...
  <div class="sc-stage-inline">
    ...
    <input type="number" class="sc-number sc-num-narrow" data-path="${ps}" data-stage-field="triggerMs" min="100" max="10000" value="${s.triggerMs}">
    <span class="sc-unit">ms</span>` : '';

        // Only 'none' (carrier/relay) stages can have children and trigger logic
        const connector = s.element === 'none' ? `
  <div class="sc-stage-connector">
    ...
    <button class="sc-btn sc-stage-add-child" data-path="${ps}" style="margin-left:auto">+ Child</button>
        ...
        const card = `
<div class="sc-stage-card" style="margin-left:${indent}px">
  <div class="sc-stage-header">
    <span>${icon} ${s.element === 'none' ? 'Carrier' : s.element[0].toUpperCase() + s.element.slice(1)}${s.stationary ? ' (area)' : ''}</span>
    ${canDel ? `<button class="sc-btn sc-stage-del" data-path="${ps}">${delLabel}</button>` : ''}
  </div>
  ...
  ${connector}
        ...
        for (let i = 0; i < s.children.length; i++) {
            childrenHtml += this.renderNode(s.children[i], [...path, i], depth + 1);
        }

        return card + childrenHtml;
    }

    private countDescendants(s: StageDraft): number {
        return s.children.reduce((n, c) => n + 1 + this.countDescendants(c), 0);
    }
    ...
    private handleStageField(path: number[], field: string, value: string): void {
        ...
        const sync = (a: string, b: string, val: string) => {
            const ea = this.stageTreeEl.querySelector<HTMLInputElement>(`[data-path="${ps}"][data-stage-field="${a}"]`);
            const eb = this.stageTreeEl.querySelector<HTMLInputElement>(`[data-path="${ps}"][data-stage-field="${b}"]`);
            if (ea) ea.value = val;
            if (eb) eb.value = val;
        };
        ...
        switch (field) {
            ...
            case 'duration': {
                ...
                sync('duration', 'durationN', String(s.duration));
                const en = this.stageTreeEl.querySelector<HTMLInputElement>(`[data-path="${ps}"][data-stage-field="durationN"]`);
            ...
            case 'durationN': {
                ...
                const el = this.stageTreeEl.querySelector<HTMLInputElement>(`[data-path="${ps}"][data-stage-field="duration"]`);
    ...
    private chainUpdatePreview(): void {
        ...
        const walk = (nodes: StageDraft[], depth: number) => {
            for (const s of nodes) {
                ...
                const trig = s.children.length > 0
                    ? ` → ${s.trigger}${s.trigger !== 'impact' ? ' ' + s.triggerMs + 'ms' : ''}`
                    : '';
                lines.push(`<span class="sc-proj-preview-item">${pad}${icon} ${s.stationary ? '(area)' : '(proj)'}${dmg}${trig}</span>`);
                walk(s.children, depth + 1);
            }
        };
        walk(this.stageRoots, 0);

        const mana = Math.max(1, this.chainMana(this.stageRoots));
        this.previewEl.innerHTML = `
<div class="sc-cost">Mana: <strong>${mana}</strong></div>
  ...
  <div class="sc-viz-side">
    ...
    <div class="sc-viz-hint">Drag to orbit · Click to select · Hold G: move · Hold R: rotate direction</div>
        ...
        this.viz.onProjectileSelected = (idx) => {
            ...
            this.renderProjTabs(); this.loadProjToEditor(); this.syncViz();
        };
    ...
    private bindEvents(): void {
        this.overlay.addEventListener('click', e => {
            ...
            if (t.classList.contains('sc-stage-add-child')) {
                const path = t.dataset['path']!.split(',').map(Number);
                this.getNode(path).children.push(defaultStageDraft());
                this.renderStageTree(); this.chainUpdatePreview(); return;
            }
            ...
            if (t.classList.contains('sc-stage-elem-btn')) {
                ...
                node.element = newEl;
                if (newEl !== 'none') node.children = [];  // elemental stages are terminal
                this.renderStageTree(); this.chainUpdatePreview(); return;
            }
        });
        ...
        const bindProj = (
            ...
            min: number, max: number, decimals: number, prop: keyof ProjState,
        ) => {
